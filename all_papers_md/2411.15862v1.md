...
computational costs, many researches have attempted to use implicit CoT, which does not
need LLMs to explicitly generate the intermediate steps. But there is still gap between their
...
from the model’s hidden states when it is performing implicit CoT. The results surprisingly
indicate that LLMs hardly think about intermediate steps, suggesting they may just rely
on experience rather than strict step-by-step
reasoning. Moreover, we find LLMs’ implicit
reasoning capabilities are susceptible and unstable, reaffirming the necessity of explicit CoT
...
have unveiled unprecedented capabilities in handling complex reasoning tasks. Chain-of-Thought
(CoT) prompting (Wei et al., 2022; Yu et al., 2023),
in particular, has demonstrated substantial improvements in the reasoning abilities of LLMs by explicitly mapping out intermediate reasoning steps.
Moreover, recent works of CoT training, such as
OpenAI o1 (Qin et al., 2024) further demonstrate
the power of CoT.
However, the CoT approach, despite its efficacy, it notably incurs slower inference speeds and
...
of intermediate tokens, leveraging the model’s inherent “vertical” reasoning capabilities through its
internal processing layers. For example, (Deng



et al., 2024) remove the intermediate steps and finetune the model to let model learn implicit CoT, and
...
implicit reasoning or vertical reasoning, which contrasts with the “horizontal” reasoning, i.e. typical CoT. Figure 1 shows the difference between
explicit CoT and implicit CoT. Although the concept of “implicit CoT (reasoning)” is rarely directly
mentioned, in many scenarios that require low latency, users usually ask LLMs to output the final
answer directly, which actually has forced LLMs
to adopt the implicit reasoning way.
Despite the theoretical appeal of implicit reasoning as a more efficient alternative to traditional CoT
methods, empirical evidence suggests the performance of implicit CoT still lag behind explicit CoT.
...
examine whether LLMs can recall their parametric knowledge during implicit reasoning, instead of
investigating more basic and generic forms of multistep problems such as arithmetic. So far, there is
...
questions about the nature of the implicit reasoning, such as “Are LLMs doing the same thing in
the processes of implicit and explicit CoT?” and
“Can the hidden, internal and layer-by-layer processing truly serve as an equivalent to explicit CoT
reasoning?” To answer these questions, our study
designs a elaborate set of experiments aimed at uncovering the implicit reasoning processes within a
...
Figure 1: The examples of explicit CoT and implicit CoT. Explicit CoT is commonly used, which is completed by
...
the model to direct give the answer without outputting steps, so that we can examine whether these
tasks can be addressed through implicit reasoning
and how implicit reasoning happens. The arithmetic problems has controllable number of reasoning steps, with each intermediate result being
known. By investigating the hidden states associated with the final token of the given problem
...
counter-intuitive: we find the model hardly calculates the intermediate results in implicit reasoning,
...
difficulty, we find implicit reasoning is more unstable and susceptible. This finding suggests in
implicit reasoning, the model may not strictly follow a step-by-step reasoning process, but relies
...
often directly give the correct answer of a multistep problem, especially when with a larger size,
they are not really doing step-by-step reasoning
(at least in arithmetic problems), unless adopting
explicit CoT. Implicit reasoning may just be an illusion created by LLMs’ powerful memory and rich
...
to generate 2000 different samples, and each intermediate results are record. For example, the
intermediate results of the above example should
be [8 _,_ 3 _,_ 5 _,_ 10 _,_ 9], i.e. the corresponding value of
...
finally get 20 _∗_ _num_ _ _steps_ classifiers. If the classifier of the _k_ -th hidden state shows high accuracy in
the _n_ -th step, it represents the model has calculated
the result of the _n_ -th step in the _k_ -th hidden state.
...
is actually not a specific state where the model calculates the results of the intermediate steps, even
when it correctly give a answer of the multi-step
problem. It actually skips the intermediate steps
and come up with the final result directly. Therefore, we posit that perhaps due to a large model’s
strong abstraction and memory abilities, it has
learned a large number of answers to mathematical problems during the training stage. Therefore,
it can almost directly map problems with multiple
steps to their answers through intuition and memory, thus producing a “implicit reasoning” effect.
...
reasoning and explicit reasoning, we slightly modify the problem by 2 methods: 1. reversing the
...
in implicit reasoning, the model is actually answering directly by experience and intuition, but not
...
LLMs doing implicit reasoning, and get a nontrivial finding that, unlike some previous studies
...
Zeyuan Allen-Zhu and Yuanzhi Li. [Physics of language](https://doi.org/10.48550/arXiv.2309.14402)
models: [Part 3.2, knowledge manipulation.](https://doi.org/10.48550/arXiv.2309.14402) _Preprint_,
arxiv:2309.14402 [cs].


Yuntian Deng, Yejin Choi, and Stuart Shieber. 2024.

[From explicit CoT to implicit CoT: Learning to inter-](https://doi.org/10.48550/arXiv.2405.14838)
...
[nalize CoT step by step.](https://doi.org/10.48550/arXiv.2405.14838) _Preprint_, arxiv:2405.14838
...
2023. [Implicit chain of thought reasoning via knowl-](https://doi.org/10.48550/arXiv.2311.01460)
[edge distillation.](https://doi.org/10.48550/arXiv.2311.01460) _Preprint_, arxiv:2311.01460.


Daniel Kahneman. 2011. Thinking, fast and slow. _Far-_
_rar, Straus and Giroux_ .


Yiwei Qin, Xuefeng Li, Haoyang Zou, Yixiu Liu, Shijie
Xia, Zhen Huang, Yixin Ye, Weizhe Yuan, Hector
Liu, Yuanzhi Li, and Pengfei Liu. 2024. [O1 Replica-](http://arxiv.org/abs/2410.18982)
tion Journey: [A Strategic Progress Report – Part 1.](http://arxiv.org/abs/2410.18982)
_arXiv preprint_ . ArXiv:2410.18982.


Qwen Team. 2024. Qwen2.5: A [party](http://qwenlm.github.io/blog/qwen2.5/) of foundation
[models!](http://qwenlm.github.io/blog/qwen2.5/) Section: blog.


Boshi Wang, Xiang Yue, Yu Su, and Huan Sun.

Grokked transformers [are](https://doi.org/10.48550/arXiv.2405.15071) implicit reasoners: A
mechanistic journey to [the](https://doi.org/10.48550/arXiv.2405.15071) edge of generalization.
_Preprint_, arxiv:2405.15071.
...
elicits reasoning in [large](http://papers.nips.cc/paper_files/paper/2022/hash/9d5609613524ecf4f15af0f7b31abca4-Abstract-Conference.html) language models. In _Ad-_
...
Geva, and Sebastian Riedel. [Do large language mod-](https://doi.org/10.48550/arXiv.2402.16837)
[els latently perform multi-hop reasoning?](https://doi.org/10.48550/arXiv.2402.16837) _Preprint_,
arxiv:2402.16837.


Tian Ye, Zicheng Xu, Yuanzhi Li, and Zeyuan AllenZhu. 2024. [Physics of Language Models:](https://arxiv.org/abs/2407.20311) Part 2.1,
[Grade-School Math and the Hidden Reasoning Pro-](https://arxiv.org/abs/2407.20311)
[cess.](https://arxiv.org/abs/2407.20311)


Zihan Yu, Liang He, Zhen Wu, Xinyu Dai, and Jiajun Chen. 2023. [Towards Better Chain-of-Thought](https://doi.org/10.48550/arXiv.2310.04959)
Prompting [Strategies:](https://doi.org/10.48550/arXiv.2310.04959) A Survey. _arXiv_ _preprint_ .
ArXiv:2310.04959.
